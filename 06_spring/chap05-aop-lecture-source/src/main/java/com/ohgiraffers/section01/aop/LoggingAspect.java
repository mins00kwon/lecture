...
//        }
    ...
    /* 설명: Pointcut 에 해당하는 위치가 담긴 메소드가 같은 클래스에 있으면 클래스 소속 생략 가능*/
    /* 설명: returning에 쓰인 이름이 반환값이 넘어올 때의 이름이고, 매개변수와 일치해야 한다. (result)*/
    @AfterReturning(value = "LoggingAspect.loggingAspect()", returning = "result")
    ...
    /* 설명: 어노테이션의 .throwing 속성과 어드바이스의 매개변수 이름을 일치시켜야 함 (exception) */
    ...
    *
    *   타켓 메소드를 언제 실행할지, 아니면 아예 실행하지 않은지 여부도 제어함. (필터와 유사함)
    *   하지만 타켓 메소드를 실행하는 proceed() 메소드를 잊는 경우가 자주 발생하기 때문에 사용시 주의해야 하며
    ...
    * */
    ...
    public Object logAround(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        // 타겟 메소드 동작 -> 실행 안하면 실행 안됨
        Object result=joinPoint.proceed();      // 타겟 메소드 동작

        System.out.println("Around After");

        /* 설명: 실행된 타켓 메소드를 반환 (그래야 다른 어드바이스가 다시 실행될 수 있다)*/
        return result;
    }
}
